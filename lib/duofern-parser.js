...
class DuoFernParser {
    constructor(options = {}) {
        this.frameHexLength = options.frameHexLength || 44; 
    }
    ...
    inferDeviceType(hex) {
        // Common style device id position is not always the same for every telegram.
    ...
    decodeShutterLike(states, b, deviceType) {
        // maps position from payload byte 7 bit 0..6 inverted for many shutter devices.
